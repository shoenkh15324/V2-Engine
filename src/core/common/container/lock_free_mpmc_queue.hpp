...
public:
    explicit LockFreeMpmcQueue(size_t capacity) : slots_(std::make_unique<Slot[]>(capacity)), capacity_(capacity){
        if(capacity_ == 0) throw std::invalid_argument("LockFreeMpmcQueue capacity must be greater than zero");
        for(size_t i = 0; i < capacity_; ++i){
            slots_[i].sequence.store(i, std::memory_order_relaxed);
        }
    }
    ~LockFreeMpmcQueue(){ // 모든 producer와 consumer가 종료된 뒤 파괴해야 한다.
