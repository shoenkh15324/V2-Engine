...
TEST(Actor, SendMsgByName){
    ...
    auto targetCtx = std::unique_ptr<ActorRuntime>(new ActorRuntime(std::move(targetActor), std::make_unique<Mailbox>(64), {.actorRegistry = &reg}));
    reg.add(target);
    auto senderActor = std::make_unique<TestActor>("sender", 1);
    auto* sender = senderActor.get();
    auto senderCtx = std::unique_ptr<ActorRuntime>(new ActorRuntime(std::move(senderActor), std::make_unique<Mailbox>(64), {.actorRegistry = &reg}));
    reg.add(sender);

    sender->sendMsg("target", Message::make(Tick{}));
...
TEST(Actor, SendMsgById){
    ...
    sender->sendMsg(uint64_t(42), Message::make(Tick{}));
    EXPECT_EQ(target->mailboxCount(), 1);
}
...
TEST(Actor, SendMsgUnknown){
    ...
    sender->sendMsg("nobody", Message::make(Tick{})); // should not crash
}

TEST(Actor, ReceiveMsg){
    auto actor = std::make_unique<TestActor>("a", 1);
    auto* a = actor.get();
    auto ctx = std::unique_ptr<ActorRuntime>(new ActorRuntime(std::move(actor), std::make_unique<Mailbox>(64), {}));

    EXPECT_EQ(a->mailboxCount(), 0);
    a->receiveMsg(Message::make(Tick{}));
...
TEST(Actor, StartTimer){
    ...
    auto actor = std::make_unique<TestActor>("a", 1);
    auto* a = actor.get();
    auto ctx = std::unique_ptr<ActorRuntime>(new ActorRuntime(std::move(actor), std::make_unique<Mailbox>(64), {.scheduler = &sched}));

    int id = a->startTimer(Message::make(Tick{}), 100, false);
    EXPECT_GT(id, 0);
...
TEST(Actor, StartTimerRepeating){
    ...
    int id = a->startTimer(Message::make(Tick{}), 100, true);
    EXPECT_GT(id, 0);
    EXPECT_EQ(sched.lastDelayMs, 100);
...
TEST(Actor, DestructorCancelsTimers){
    TestScheduler sched;
    {
        ...
        a->startTimer(Message::make(Tick{}), 100, false);
        a->startTimer(Message::make(Tick{}), 200, true);
