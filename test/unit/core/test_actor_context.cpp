...
TEST(ActorContext, Create){
    ...
    auto* a = actor.get();
    ActorContext ctx(std::move(actor), 64, nullptr, nullptr, nullptr);
...
TEST(ActorContext, CreateWithRegistry){
    ...
    auto actor = std::make_unique<TestActor>("a", 1);
    auto* a = actor.get();
    ActorContext ctx(std::move(actor), 64, nullptr, nullptr, &reg);
    reg.add(a);
...
TEST(ActorContext, Enqueue){
    ActorContext ctx(std::make_unique<TestActor>("a", 1), 64, nullptr, nullptr, nullptr);
    ctx.enqueue(Tick{});
...
TEST(ActorContext, RunMaxBatch){
    Dispatcher d(1);
    ActorContext ctx(std::make_unique<TestActor>("a", 1), 64, &d, nullptr, nullptr);
    auto* a = static_cast<TestActor*>(ctx.actor());
...
TEST(ActorContext, DestructorRemovesFromRegistry){
    ActorRegistry reg;
    {
        ActorContext ctx(std::make_unique<TestActor>("a", 1), 64, nullptr, nullptr,&reg);
        reg.add(ctx.actor());
