...
동시성(concurrency) 환경에서 뮤텍스 기반 메일박스의 **동시 push 성능**을 측정합니다.
...
**해석:**
- 8개까지는 거의 선형적으로 증가
- 16개에서 포화/감소 → 뮤텍스 경쟁이 포화점에 도달
- 뮤텍스의 **critical section** (push 연산)이 극히 짧아서 높은 동시성 수용
...
이 벤치마크로 **뮤텍스의 실제 오버헤드를 추정**할 수 있습니다.
...
Worker 스레드:
  sema_.acquire()  → Mailbox::pop() [뮤텍스]  → handle()
```

### 동시 push의 안전성

`Mailbox::push()`는 뮤텍스로 보호됩니다. 따라서:
- 한 번에 하나의 프로듀서만 push 가능
- 뮤텍스 해제 후 다른 프로듀서가 push
- 메일박스 데이터 일관성 보장

## 결론

- 뮤텍스 기반 메일박스는 **동시 push에 강함** (critical section이 짧음)
- 8개 프로듀서에서 ~470K msgs/sec → 초당 100만 메시지 생성 시스템에도 충분
- 뮤텍스 오버헤드는 push당 ~50-100ns 수준
