...
최초 설계 스케치대로 구현하되 스케치의 정확성 결함 3건을 교정:
...
- **Site B** `graceSpin`: finalize 초입, 슬롯 반납 **전** `tokenGraceNs`(5μs) 메일박스 폴링 — 유예 중 도착 메시지를 기존 토큰이 흡수 (경합 창 추가 없음, 별도 계획이던 '토큰 유예' 과제를 본 항목으로 흡수)
...
> **핵심**: `actors = workers`일 때 최악. 단순 fence 최적화로는 해결 불가. *(2026-08-24: 근본 원인은 futex 수면 진동으로 확정 — 진행 현황 ② 참고)*
