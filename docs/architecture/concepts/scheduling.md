...
>
> 관련 코드: `src/core/actor_system/runtime/dispatcher/`, `src/core/actor_system/runtime/actor_runtime/`
>
> 로드맵 배경: `docs/plans/roadmap.md` 4-4(로드 밸런싱), 4-5(스케일링 붕괴 해소)
...
3. 잦은 잠들기/깨어나기(OS futex)가 성능을 깎아먹으므로, **스핀 → 파킹** 계층형 대기로 최소화한다.
...
- 워커는 호출표 하나로 메일박스를 **배치 단위**로 처리 — 깨우기 횟수 최소화
...
sequenceDiagram
    ...
    alt 이전값 0 → 토큰 없었음 (내가 발행)
        D->>D: 4. 워커 큐에 토큰 push<br/>pendingWork++ , 세마포어 깨우기
...
토큰이 매번 죽고 태어나면서 **일하는 시간보다 깨어나는 시간이 커집니다.**

실측 예 (w=4, a=16): 평균 배치 18.5개 → 1.41개, 워커 유휴율 0.4% → **94%.**
사이클 8.6μs 중 7.4μs가 "잠에서 깨는 비용".
...
- **단일 프로듀서 캡**: 부하 발행도 1스레드면 시스템 전체가 그 속도로 묶임(~24M/s).
  양의 스케일링 입증엔 `--producers N` 필수. 엔진은 이미 MPSC라 복수 발행자 지원함.
- **완료 폴링 오염**: 드라이버가 처리량 카운터를 무휴면 읽으면 워커의 캐시라인을 계속 무효화해
  고W에서 과소 측정 → 폴링 사이 50μs 휴면 삽입.
...
| busyMs | 실제 run() 시간 합 | — |
| idleMs | acquire 대기 시간 합 | 작을수록 좋음 (크면 깨우기 비용 의심) |
| dedup | 중복 토큰 발행 거절 수 | 트래픽 몰림 정도의 자연 스케일 |

판별 규칙: **idleMs↑ = 깨우기 비용 문제** / **busyMs↑ + 낮은 처리량 = 캐시·라우팅 문제**.
